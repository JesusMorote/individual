...
//Incluyo el fichero que permite consultar el stock de una referencia dada de GGOYA
include_once('consulta_stock_ggoya.inc.php');

//Obtengo la fecha actual, en el momento en que se ha iniciado la ejecución de consulta_stock_todos_ggoya.php
...
//Obtengo las referencias almacenadas en la tabla refs_ggoya que no tengan correspondencia con una referencia de makito
$consulta_referencias = "SELECT DISTINCT refs_ggoya_referencia FROM refs_ggoya
                                WHERE refs_ggoya_referencia NOT IN
                                    (SELECT DISTINCT referencias_makito_vs_competidores_ggy FROM referencias_makito_vs_competidores
...
$consulta_lote = "SELECT DISTINCT refs_ggoya_referencia
                    FROM refs_ggoya
                        WHERE refs_ggoya_referencia NOT IN
                                    (SELECT DISTINCT referencias_makito_vs_competidores_ggy FROM referencias_makito_vs_competidores
                                        WHERE referencias_makito_vs_competidores_ggy NOT LIKE '')
...
$nombre_fichero_log = "../log/log_stock_todos_ggoya_".$fecha_actual_inicio."_".date('H-i-s').".txt";
$log = fopen($nombre_fichero_log, "w");
fwrite($log, "CONSULTA DE STOCK DE TODOS LOS ARTÍCULOS DE GGOYA:\r\nProceso iniciado el ".$fecha_actual_inicio.", a las ".$hora_actual_inicio."\r\n***************************************************************************\r\n\r\n");
fclose($log);
...
while (!($resultado_consulta_lote->EOF)) {
    ...
    $ref_ggoya = $resultado_consulta_lote->fields['refs_ggoya_referencia'];
    
    //Realizo las consultas de stock AGREGADO de las distintas referencias en GGOYA del lote
        ...
        if($ref_ggoya != "") {
            try {
                ...
                if($stock_ggoya == -2) { //Se ha producido un error al consultar el stock de esa referencia
                    ...
                    fwrite($log, "#### ERROR al procesar la ref.".$ref_ggoya." - ".$ref_ggoya." (GGOYA)\r\n");
                    fclose($log);
                }
    ...
    $log = fopen($nombre_fichero_log, "a");
    fwrite($log, "Ref: ".$ref_ggoya." / Inicio: ".$fecha_actual_inicio_ref." - ".$hora_actual_inicio_ref." / Fin: ".$fecha_actual_fin_ref." - ".$hora_actual_fin_ref." / ".$resultado_proceso_ref."\r\n");
    fclose($log);
...
$consulta_referencias_erroneas = "SELECT * FROM stock_ggoya
                                    WHERE stock_ggoya_stock = -2
                                        AND (stock_ggoya_fecha BETWEEN '$fecha_actual_inicio' AND '$fecha_actual_fin')
                                        AND (stock_ggoya_hora BETWEEN '$hora_actual_inicio' AND '$hora_actual_fin');";
...
if(count($referencias_erroneas) != 0) {
    ...
    while (!($referencias_erroneas->EOF)) {
        ...
        $referencia_ggoya_con_errores = $referencias_erroneas->fields['stock_ggoya_ref'];
        $referencia_ggoya_con_errores_stock_ggoya_almacenado = $referencias_erroneas->fields['stock_ggoya_stock'];
        ...
        $ref_ggoya = $referencia_ggoya_con_errores;
        
        //CONSULTO Y ALMACENO DE NUEVO EL STOCK EN GGOYA (si su stock almacenado en la primera vuelta es -2)
        if($referencia_ggoya_con_errores_stock_ggoya_almacenado == -2) {
            try {
                $stock_ggoya = consulta_stock_ggoya($ref_ggoya);
