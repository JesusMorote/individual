...
//Incluyo el fichero que permite consultar el stock de una referencia dada de GIVING
include_once('consulta_stock_giving.inc.php');

//Obtengo la fecha actual, en el momento en que se ha iniciado la ejecución de consulta_stock_todos_giving.php
...
//Obtengo las referencias almacenadas en la tabla refs_giving que no tengan correspondencia con una referencia de makito
$consulta_referencias = "SELECT DISTINCT refs_giving_referencia FROM refs_giving
                                WHERE refs_giving_referencia NOT IN
                                    (SELECT DISTINCT referencias_makito_vs_competidores_gvng FROM referencias_makito_vs_competidores
...
$consulta_lote = "SELECT DISTINCT refs_giving_referencia
                    FROM refs_giving
                        WHERE refs_giving_referencia NOT IN
                                    (SELECT DISTINCT referencias_makito_vs_competidores_gvng FROM referencias_makito_vs_competidores
                                        WHERE referencias_makito_vs_competidores_gvng NOT LIKE '')
...
$consulta_lote = "SELECT DISTINCT refs_giving_referencia
                    FROM refs_giving
                    WHERE refs_giving_referencia LIKE '5401'
                       OR refs_giving_referencia LIKE '6478'
                    ORDER BY refs_giving_referencia ASC;";
*/
...
$nombre_fichero_log = "../log/log_stock_todos_giving_".$fecha_actual_inicio."_".date('H-i-s').".txt";
$log = fopen($nombre_fichero_log, "w");
fwrite($log, "CONSULTA DE STOCK DE TODOS LOS ARTÍCULOS DE GIVING:\r\nProceso iniciado el ".$fecha_actual_inicio.", a las ".$hora_actual_inicio."\r\n***************************************************************************\r\n\r\n");
fclose($log);
...
while (!($resultado_consulta_lote->EOF)) {
    ...
    $ref_giving = $resultado_consulta_lote->fields['refs_giving_referencia'];
    
    //Realizo las consultas de stock AGREGADO de las distintas referencias en GIVING del lote
        ...
        if($ref_giving != "") {
            ...
            try {
                ...
                if($stock_giving == -2) { //Se ha producido un error al consultar el stock de esa referencia
                    ...
                    fwrite($log, "#### ERROR al procesar la ref.".$ref_giving." - ".$ref_giving." (GIVING)\r\n");
                    fclose($log);
                }
    ...
    $log = fopen($nombre_fichero_log, "a");
    fwrite($log, "Ref: ".$ref_giving." / Inicio: ".$fecha_actual_inicio_ref." - ".$hora_actual_inicio_ref." / Fin: ".$fecha_actual_fin_ref." - ".$hora_actual_fin_ref." / ".$resultado_proceso_ref."\r\n");
    fclose($log);
...
$consulta_referencias_erroneas = "SELECT * FROM stock_giving
                                    WHERE stock_giving_stock = -2
                                        AND (stock_giving_fecha BETWEEN '$fecha_actual_inicio' AND '$fecha_actual_fin')
                                        AND (stock_giving_hora BETWEEN '$hora_actual_inicio' AND '$hora_actual_fin');";
...
if(count($referencias_erroneas) != 0) {
    ...
    while (!($referencias_erroneas->EOF)) {
        ...
        $referencia_giving_con_errores = $referencias_erroneas->fields['stock_giving_ref'];
        $referencia_giving_con_errores_stock_giving_almacenado = $referencias_erroneas->fields['stock_giving_stock'];
        ...
        $ref_giving = $referencia_giving_con_errores;
        
        //CONSULTO Y ALMACENO DE NUEVO EL STOCK EN GIVING (si su stock almacenado en la primera vuelta es -2)
        
        if($referencia_giving_con_errores_stock_giving_almacenado == -2) {
