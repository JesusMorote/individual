...
if(!isset($_SESSION['user'])) { //Se intenta acceder a la aplicación sin un login correcto
    ...
    if (isset($_SESSION['filtro_mob'])) {
        $filtro = $_SESSION['filtro_mob'];
    } else {
        $filtro = "todos";
    }
    ...
    if ($filtro == "todos") {
         $consulta_referencias = "SELECT DISTINCT refs_mob_referencia_raiz
                                    FROM refs_mob
                                    ORDER BY refs_mob_referencia_raiz ASC
                                    LIMIT $ref_por_pag OFFSET $offset;";
        
        $consulta_referencias_prev = "SELECT DISTINCT refs_mob_referencia_raiz
                                        FROM refs_mob;";
        ...
        if(substr($filtro, 0, 2) == "ZZ") { //se ha seleccionado una subcategoría
            ...
            $consulta_referencias = "SELECT DISTINCT refs_mob_referencia_raiz
                                    FROM refs_mob
                                    WHERE refs_mob_subcategoria LIKE '$subcategoria'
                                    ORDER BY refs_mob_referencia_raiz ASC
                                    LIMIT $ref_por_pag OFFSET $offset;";
            ...
            $consulta_referencias = "SELECT DISTINCT refs_mob_referencia_raiz
                                    FROM refs_mob
                                    WHERE refs_mob_categoria LIKE '$categoria'
                                    ORDER BY refs_mob_referencia_raiz ASC
                                    LIMIT $ref_por_pag OFFSET $offset;";
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            <h4 class="enmarcado2 indentado text-center">Listado de Referencias de MOB</h4>
            ...
            <div class="col-md-4 col-md-offset-3 text-center margin_inf">
                ...
                <div class="form-group">                
                    <select class="form-control" name="filtro" id="filtro">
                        ...
                        $consulta_categorias = "SELECT DISTINCT refs_mob_categoria
                                                    FROM refs_mob
                                                    ORDER BY refs_mob_categoria ASC";
    
                        $resultado_consulta_categorias = $bd->Execute($consulta_categorias);
                        
                        while (!($resultado_consulta_categorias->EOF)) {
                            $nombre_categoria = $resultado_consulta_categorias->fields['refs_mob_categoria'];
                            
                            ?>
                            <option value="<?php echo $nombre_categoria; ?>" id="<?php echo preg_replace('[\)]', "", preg_replace('[\(]', "", preg_replace('[&]', "", preg_replace('[,]', "", preg_replace('[ñ]', "", preg_replace('[/]', "", preg_replace('[\s+]', "", $nombre_categoria))))))); ?>" class="capitalize categoria"><?php echo $nombre_categoria; ?></option>
                            <?php
                            
                            $consulta_subcategorias = "SELECT DISTINCT refs_mob_subcategoria
                                                        FROM refs_mob
                                                        WHERE refs_mob_categoria LIKE '$nombre_categoria'
                                                        ORDER BY refs_mob_subcategoria ASC";

                            $resultado_consulta_subcategorias = $bd->Execute($consulta_subcategorias);
                            
                            while (!($resultado_consulta_subcategorias->EOF)) {
                                $nombre_subcategoria = $resultado_consulta_subcategorias->fields['refs_mob_subcategoria'];
                                
                                ?>
                                <option value="<?php echo "ZZ".$nombre_subcategoria; ?>" id="<?php echo "ZZ".preg_replace('[\)]', "", preg_replace('[\(]', "", preg_replace('[&]', "", preg_replace('[,]', "", preg_replace('[ñ]', "", preg_replace('[/]', "", preg_replace('[\s+]', "", $nombre_subcategoria))))))); ?>" class="capitalize"> - <?php echo $nombre_subcategoria; ?></option>
                                <?php
            ...
            <nav>
                <ul class="pager linea_nav">
                    ...
                    if($pag == 1) {
                        ...
                        ?>
                        <li>
                            <a href="inicio.php?id=listar_referencias_mob&pag=<?php echo ($pag-1); ?>">
                                <span aria-hidden="true">&laquo;</span> Pág. Anterior
                            </a>
                        </li>
                        <?php
                    ...
                    if($pag == $num_paginas) {
                        ...
                        ?>
                        <li>
                            <a href="inicio.php?id=listar_referencias_mob&pag=<?php echo ($pag+1); ?>" aria-label="Previous">
                                Pág. Siguiente <span aria-hidden="true">&raquo;</span>
                            </a>
                        </li>
                        <?php
            ...
            <div class="col-md-12">
                ...
                <table class="table table-bordered table-condensed table-striped">
                    <tr id="cabecera_listado">
                        ...
                        <th class="info celdacentradahor celdacentradavert"><i class="glyphicon glyphicon-barcode"></i> Ref. Raiz MOB</th>
                ...
                while (!($resultado_consulta_referencias->EOF)) {
                    $articulo_ref = $resultado_consulta_referencias->fields['refs_mob_referencia_raiz'];
                    
                    $consulta_datos_referencia = "SELECT * FROM refs_mob
                                                    WHERE refs_mob_referencia_raiz LIKE '$articulo_ref'
                                                    LIMIT 1;";
                    $resultado_consulta_datos_referencia = $bd->Execute($consulta_datos_referencia);
                    
                    $articulo_url_imagen = $resultado_consulta_datos_referencia->fields['refs_mob_url_imagen'];
                    ...
                    $consulta_num_ref_hijas = "SELECT COUNT(*) FROM refs_mob
                                                    WHERE refs_mob_referencia_raiz LIKE '$articulo_ref';";
                    ...
                    if($num_ref_hijas > 1) {
                        ...
                        $consulta_ref_hijas = "SELECT refs_mob_nombre FROM refs_mob
                                                    WHERE refs_mob_referencia_raiz LIKE '$articulo_ref';";
                        $resultado_consulta_ref_hijas = $bd->Execute($consulta_ref_hijas);
                        
                        $nombre_ref_hija1 = $resultado_consulta_ref_hijas->fields['refs_mob_nombre'];
                        $resultado_consulta_ref_hijas->MoveNext();
                        $nombre_ref_hija2 = $resultado_consulta_ref_hijas->fields['refs_mob_nombre'];
                        ...
                        while($coincide_palabra and ($indice < $longitud_array_nombre_ref_hija1)) {
                            if($array_nombre_ref_hija1[$indice] == $array_nombre_ref_hija2[$indice]) {
                                $articulo_nombre = $articulo_nombre.$array_nombre_ref_hija1[$indice]." ";
                                $indice++;
                            } else {
                        ...
                        $articulo_nombre = $resultado_consulta_datos_referencia->fields['refs_mob_nombre'];
                    }
                    
                    $articulo_categoria = $resultado_consulta_datos_referencia->fields['refs_mob_categoria'];
                    $articulo_subcategoria = $resultado_consulta_datos_referencia->fields['refs_mob_subcategoria'];
                    ...
                    <tr>
                        <td class="celdacentradahor celdacentradavert">
                            ...
                            if($urlexists) {
                                ?>
                                <img src="<?php echo $articulo_url_imagen; ?>" width="50px" class="enmarcado3 miniatura">
                                <?php
                            } else {
                                ?>
                        ...
                        <td class="celdacentradahor celdacentradavert mas_info celda_homogenea">
                            ...
                            if($articulo_ref != "") {
                                ?>
                                <br>
                                <a href="../inc/muestra_web_competidor.inc.php?ref=<?php echo $articulo_ref; ?>&comp=mob" target="_blank">
                        ...
                        <td class="celdacentradahor celdacentradavert">
                            <a href="inicio.php?id=variacion_stock_mob&ref=<?php echo $articulo_ref; ?>" target="_blank">
            ...
            <nav aria-label="Page navigation" class="text-center">
                <ul class="pagination linea_nav2">
                    ...
                    for($i=1; $i<= $num_paginas; $i++) {
                        ...
                        if($i == $pag) {
                            ?>
                            <li class="active"><a href="inicio.php?id=listar_referencias_mob&pag=<?php echo $i; ?>" class="btn_pg_<?php echo $i; ?>"><?php echo $i; ?></a></li>
                            <?php
                        } else {
                            ?>
                            <li><a href="inicio.php?id=listar_referencias_mob&pag=<?php echo $i; ?>" class="btn_pg_<?php echo $i; ?>"><?php echo $i; ?></a></li>
                            <?php
                        }
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#<?php echo preg_replace('[\)]', "", preg_replace('[\(]', "", preg_replace('[&]', "", preg_replace('[,]', "", preg_replace('[ñ]', "", preg_replace('[/]', "", preg_replace('[\s+]', "",$filtro))))))); ?>").attr("selected", "selected");
            $("#<?php echo preg_replace('[\)]', "", preg_replace('[\(]', "", preg_replace('[&]', "", preg_replace('[,]', "", preg_replace('[ñ]', "", preg_replace('[/]', "", preg_replace('[\s+]', "",$filtro))))))); ?>").css("background-color", "#337ab7");
            $("#<?php echo preg_replace('[\)]', "", preg_replace('[\(]', "", preg_replace('[&]', "", preg_replace('[,]', "", preg_replace('[ñ]', "", preg_replace('[/]', "", preg_replace('[\s+]', "",$filtro))))))); ?>").css("color", "white");
