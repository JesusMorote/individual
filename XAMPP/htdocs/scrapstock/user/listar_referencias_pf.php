...
if(!isset($_SESSION['user'])) { //Se intenta acceder a la aplicación sin un login correcto
    ...
    if (isset($_SESSION['filtro_pf'])) {
        $filtro = $_SESSION['filtro_pf'];
    } else {
        $filtro = "todos";
    }
    ...
    if ($filtro == "todos") {
         $consulta_referencias = "SELECT DISTINCT refs_pf_referencia
                                    FROM refs_pf
                                    ORDER BY refs_pf_referencia ASC
                                    LIMIT $ref_por_pag OFFSET $offset;";
        
        $consulta_referencias_prev = "SELECT DISTINCT refs_pf_referencia
                                        FROM refs_pf;";
        ...
        if(substr($filtro, 0, 2) == "ZZ") { //se ha seleccionado una subcategoría
            ...
            $consulta_referencias = "SELECT DISTINCT refs_pf_referencia
                                    FROM refs_pf
                                    WHERE refs_pf_subcategoria LIKE '$subcategoria'
                                    ORDER BY refs_pf_referencia ASC
                                    LIMIT $ref_por_pag OFFSET $offset;";
            ...
            $consulta_referencias = "SELECT DISTINCT refs_pf_referencia
                                    FROM refs_pf
                                    WHERE refs_pf_categoria LIKE '$categoria'
                                    ORDER BY refs_pf_referencia ASC
                                    LIMIT $ref_por_pag OFFSET $offset;";
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            <h4 class="enmarcado2 indentado text-center">Listado de Referencias de PF</h4>
            ...
            <div class="col-md-4 col-md-offset-3 text-center margin_inf">
                ...
                <div class="form-group">                
                    <select class="form-control" name="filtro" id="filtro">
                        ...
                        $consulta_categorias = "SELECT DISTINCT refs_pf_categoria
                                                    FROM refs_pf
                                                    ORDER BY refs_pf_categoria ASC";
    
                        $resultado_consulta_categorias = $bd->Execute($consulta_categorias);
                        
                        while (!($resultado_consulta_categorias->EOF)) {
                            $nombre_categoria = $resultado_consulta_categorias->fields['refs_pf_categoria'];
                            ...
                            $consulta_subcategorias = "SELECT DISTINCT refs_pf_subcategoria
                                                        FROM refs_pf
                                                        WHERE refs_pf_categoria LIKE '$nombre_categoria'
                                                        ORDER BY refs_pf_subcategoria ASC";

                            $resultado_consulta_subcategorias = $bd->Execute($consulta_subcategorias);
                            
                            while (!($resultado_consulta_subcategorias->EOF)) {
                                $nombre_subcategoria = $resultado_consulta_subcategorias->fields['refs_pf_subcategoria'];
            ...
            <nav>
                <ul class="pager linea_nav">
                    ...
                    if($pag == 1) {
                        ...
                        ?>
                        <li>
                            <a href="inicio.php?id=listar_referencias_pf&pag=<?php echo ($pag-1); ?>">
                                <span aria-hidden="true">&laquo;</span> Pág. Anterior
                            </a>
                        </li>
                        <?php
                    ...
                    if($pag == $num_paginas) {
                        ...
                        ?>
                        <li>
                            <a href="inicio.php?id=listar_referencias_pf&pag=<?php echo ($pag+1); ?>" aria-label="Previous">
                                Pág. Siguiente <span aria-hidden="true">&raquo;</span>
                            </a>
                        </li>
                        <?php
            ...
            <div class="col-md-12">
                ...
                <table class="table table-bordered table-condensed table-striped">
                    <tr id="cabecera_listado">
                        ...
                        <th class="info celdacentradahor celdacentradavert"><i class="glyphicon glyphicon-barcode"></i> Ref. PF</th>
                ...
                while (!($resultado_consulta_referencias->EOF)) {
                    $articulo_ref = $resultado_consulta_referencias->fields['refs_pf_referencia'];
                    
                    $consulta_datos_referencia = "SELECT * FROM refs_pf
                                                    WHERE refs_pf_referencia LIKE '$articulo_ref'
                                                    LIMIT 1;";
                    $resultado_consulta_datos_referencia = $bd->Execute($consulta_datos_referencia);
                    
                    $articulo_url_imagen = $resultado_consulta_datos_referencia->fields['refs_pf_url_imagen'];
                    
                    $articulo_nombre = $resultado_consulta_datos_referencia->fields['refs_pf_nombre'];$articulo_categoria = $resultado_consulta_datos_referencia->fields['refs_pf_categoria'];
                    $articulo_subcategoria = $resultado_consulta_datos_referencia->fields['refs_pf_subcategoria'];
            ...
            <nav aria-label="Page navigation" class="text-center">
                <ul class="pagination linea_nav2">
                    ...
                    for($i=1; $i<= $num_paginas; $i++) {
                        ...
                        if($i == $pag) {
                            ?>
                            <li class="active"><a href="inicio.php?id=listar_referencias_pf&pag=<?php echo $i; ?>" class="btn_pg_<?php echo $i; ?>"><?php echo $i; ?></a></li>
                            <?php
                        } else {
                            ?>
                            <li><a href="inicio.php?id=listar_referencias_pf&pag=<?php echo $i; ?>" class="btn_pg_<?php echo $i; ?>"><?php echo $i; ?></a></li>
                            <?php
                        }
