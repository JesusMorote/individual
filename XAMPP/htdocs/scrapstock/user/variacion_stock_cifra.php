...
if(!isset($_SESSION['user'])) { //Se intenta acceder a la aplicación sin un login correcto
    ...
    $consulta_referencias_cifra = "SELECT DISTINCT refs_cifra_referencia_raiz
                                    FROM refs_cifra;";
    $resultado_consulta_referencias_cifra = $bd->Execute($consulta_referencias_cifra);
    ...
    if(isset($_GET['ref'])) {
        ...
        if(isset($_POST['fecha_inicio'])) {
            ...
            switch ($dia_semana_actual) {
                case "1": //Es lunes
                    $fecha_inicio = date('Y-m-d', strtotime($fecha_actual) - (86400 * 7));
                    $fecha_fin = date('Y-m-d', strtotime($fecha_actual) - (86400 * 1));
                    break;
                case "2": //Es martes
                    $fecha_inicio = date('Y-m-d', strtotime($fecha_actual) - (86400 * 8));
                    $fecha_fin = date('Y-m-d', strtotime($fecha_actual) - (86400 * 2));
                    break;
                case "3": //Es miércoles
                    $fecha_inicio = date('Y-m-d', strtotime($fecha_actual) - (86400 * 9));
                    $fecha_fin = date('Y-m-d', strtotime($fecha_actual) - (86400 * 3));
                    break;
                case "4": //Es jueves
                    $fecha_inicio = date('Y-m-d', strtotime($fecha_actual) - (86400 * 10));
                    $fecha_fin = date('Y-m-d', strtotime($fecha_actual) - (86400 * 4));
                    break;
                case "5": //Es viernes
                    $fecha_inicio = date('Y-m-d', strtotime($fecha_actual) - (86400 * 11));
                    $fecha_fin = date('Y-m-d', strtotime($fecha_actual) - (86400 * 5));
                    break;
                case "6": //Es sábado
                    $fecha_inicio = date('Y-m-d', strtotime($fecha_actual) - (86400 * 12));
                    $fecha_fin = date('Y-m-d', strtotime($fecha_actual) - (86400 * 6));
                    break;
                case "7": //Es domingo
                    $fecha_inicio = date('Y-m-d', strtotime($fecha_actual) - (86400 * 13));
                    $fecha_fin = date('Y-m-d', strtotime($fecha_actual) - (86400 * 7));
                    break;
            }
        ...
        //RECUPERO LOS DATOS QUE HAYA ALMACENADOS PARA LA REFERENCIA DESEADA ENTRE LAS FECHAS ESPECIFICADAS EN LOS SELECTORES DE FECHA
        $consulta_stocks = "SELECT * FROM stock_makito_vs_todos
                                WHERE stock_makito_vs_todos_fecha IN
                                    (SELECT DISTINCT stock_makito_vs_todos_fecha FROM stock_makito_vs_todos
                                        ...
                                        AND stock_makito_vs_todos_fecha BETWEEN '$fecha_inicio' AND '$fecha_fin')
        ...
        */
        ...
        $incremento_en_stock_cifra = false;
        $incremento_en_stock_giving = false;
        $incremento_en_stock_mob = false;
        $incremento_en_stock_ps = false;
        $incremento_en_stock_pf = false;
        $incremento_en_stock_ggoya = false;
        ...
        while ($dia < ($dias_diferencia+1)) {
            ...
            if($resultado_consulta_stocks_dia->fields['stock_makito_vs_todos_fecha'] != "") {
                ...
                if($dia != 0) {
                    if(($stock_cifra_dia - $array_stocks_dias[$dia-1][1]) > 0) {
                        $incremento_en_stock_cifra = true;
                    }
                    
                    if(($stock_giving_dia - $array_stocks_dias[$dia-1][2]) > 0) {
                        $incremento_en_stock_giving = true;
                    }
                    
                    if(($stock_mob_dia - $array_stocks_dias[$dia-1][3]) > 0) {
                        $incremento_en_stock_mob = true;
                    }
                    
                    if(($stock_ps_dia - $array_stocks_dias[$dia-1][4]) > 0) {
                        $incremento_en_stock_ps = true;
                    }
                    
                    if(($stock_pf_dia - $array_stocks_dias[$dia-1][5]) > 0) {
                        $incremento_en_stock_pf = true;
                    }
                    
                    if(($stock_ggoya_dia - $array_stocks_dias[$dia-1][6]) > 0) {
                        $incremento_en_stock_ggoya = true;
                    }
                }
                ...
                $consulta_existe_dia_anterior = "SELECT COUNT(*) FROM stock_makito_vs_todos
                                                   WHERE stock_makito_vs_todos_fecha < '$fecha_dia'
                                                      AND stock_makito_vs_todos_ref_makito LIKE '$ref'
                                                   ORDER BY stock_makito_vs_todos_fecha;";

                $resultado_consulta_existe_dia_anterior= $bd->Execute($consulta_existe_dia_anterior);

                $existe_dia_anterior= $resultado_consulta_existe_dia_anterior->fields[0];
                
                if($dia == 0) {
                    if($existe_dia_anterior > 0) {
                        $consulta_stocks_dia_anterior_mas_cercano = "SELECT stock_makito_vs_todos_stock_cifra FROM stock_makito_vs_todos
                                                                        WHERE stock_makito_vs_todos_fecha < '$fecha_dia'
                                                                            AND stock_makito_vs_todos_ref_makito LIKE '$ref'
                                                                        ORDER BY stock_makito_vs_todos_fecha DESC LIMIT 1;";

                        $resultado_consulta_stocks_dia_anterior_mas_cercano = $bd->Execute($consulta_stocks_dia_anterior_mas_cercano);
                        
                        $stock_cifra_dia = $resultado_consulta_stocks_dia_anterior_mas_cercano->fields[0];
                ...
                if($dia == 0) {
                    if($existe_dia_anterior > 0) {
                        $consulta_stocks_dia_anterior_mas_cercano = "SELECT stock_makito_vs_todos_stock_giving FROM stock_makito_vs_todos
                                                                        WHERE stock_makito_vs_todos_fecha < '$fecha_dia'
                                                                            AND stock_makito_vs_todos_ref_makito LIKE '$ref'
                                                                        ORDER BY stock_makito_vs_todos_fecha DESC LIMIT 1;";
                        ...
                        $stock_giving_dia = $resultado_consulta_stocks_dia_anterior_mas_cercano->fields[0];
                ...
                if($dia == 0) {
                    if($existe_dia_anterior > 0) {
                        $consulta_stocks_dia_anterior_mas_cercano = "SELECT stock_makito_vs_todos_stock_mob FROM stock_makito_vs_todos
                                                                        WHERE stock_makito_vs_todos_fecha < '$fecha_dia'
                                                                            AND stock_makito_vs_todos_ref_makito LIKE '$ref'
                                                                        ORDER BY stock_makito_vs_todos_fecha DESC LIMIT 1;";
                        ...
                        $stock_mob_dia = $resultado_consulta_stocks_dia_anterior_mas_cercano->fields[0];
                ...
                if($dia == 0) {
                    if($existe_dia_anterior > 0) {
                        $consulta_stocks_dia_anterior_mas_cercano = "SELECT stock_makito_vs_todos_stock_ps FROM stock_makito_vs_todos
                                                                        WHERE stock_makito_vs_todos_fecha < '$fecha_dia'
                                                                            AND stock_makito_vs_todos_ref_makito LIKE '$ref'
                                                                        ORDER BY stock_makito_vs_todos_fecha DESC LIMIT 1;";
                        ...
                        $stock_ps_dia = $resultado_consulta_stocks_dia_anterior_mas_cercano->fields[0];
                ...
                if($dia == 0) {
                    if($existe_dia_anterior > 0) {
                        $consulta_stocks_dia_anterior_mas_cercano = "SELECT stock_makito_vs_todos_stock_pf FROM stock_makito_vs_todos
                                                                        WHERE stock_makito_vs_todos_fecha < '$fecha_dia'
                                                                            AND stock_makito_vs_todos_ref_makito LIKE '$ref'
                                                                        ORDER BY stock_makito_vs_todos_fecha DESC LIMIT 1;";
                        ...
                        $stock_pf_dia = $resultado_consulta_stocks_dia_anterior_mas_cercano->fields[0];
                ...
                if($dia == 0) {
                    if($existe_dia_anterior > 0) {
                        $consulta_stocks_dia_anterior_mas_cercano = "SELECT stock_makito_vs_todos_stock_ggoya FROM stock_makito_vs_todos
                                                                        WHERE stock_makito_vs_todos_fecha < '$fecha_dia'
                                                                            AND stock_makito_vs_todos_ref_makito LIKE '$ref'
                                                                        ORDER BY stock_makito_vs_todos_fecha DESC LIMIT 1;";
                        ...
                        $stock_ggoya_dia = $resultado_consulta_stocks_dia_anterior_mas_cercano->fields[0];
        ...
        <div class="container" id="cuerpo_home">
            ...
            <!--Selector de Fechas de Inicio y Fin de la consulta-->
            <?php
                $consulta_min_fecha_datos = "SELECT MIN(stock_makito_vs_todos_fecha) FROM stock_makito_vs_todos
                                                WHERE stock_makito_vs_todos_ref_makito LIKE '$ref';";
        
                $resultado_consulta_min_fecha_datos = $bd->Execute($consulta_min_fecha_datos);
        
                $min_fecha_datos = $resultado_consulta_min_fecha_datos->fields[0];
        
                $consulta_max_fecha_datos = "SELECT MAX(stock_makito_vs_todos_fecha) FROM stock_makito_vs_todos
                                                WHERE stock_makito_vs_todos_ref_makito LIKE '$ref';";
        
                $resultado_consulta_max_fecha_datos = $bd->Execute($consulta_max_fecha_datos);
        
                $max_fecha_datos = $resultado_consulta_max_fecha_datos->fields[0];
            ?>
            <form action="" method="post" class="form-horizontal">
            <div class="row form-group">
                <div class="col-md-3 col-md-offset-3">
                    <label for="fecha_inicio" class="sr-only">Stocks desde:</label>
                    <div class="input-group">
                        <div class="input-group-addon">Stocks desde:</div>
                        <input class="fecha" type="date" class="form-control" name="fecha_inicio" id="fecha_inicio" min="<?php echo $min_fecha_datos; ?>" max="<?php echo $max_fecha_datos; ?>" value="<?php echo $fecha_inicio_datos; ?>" required>
                    </div>
                </div>
                <div class="col-md-3">
                    <label for="fecha_fin" class="sr-only">Stocks hasta:</label>
                    <div class="input-group">
                        <div class="input-group-addon">Stocks hasta:</div>
                        <input class="fecha" type="date" class="form-control" name="fecha_fin" id="fecha_fin" min="<?php echo $min_fecha_datos; ?>" max="<?php echo $max_fecha_datos; ?>" value="<?php echo $fecha_fin_datos; ?>" required>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-4 col-md-offset-4">
                    <button type="submit" class="btn btn-primary btn-block" id="btn_filtrar_fechas"><i class="fa fa-filter"></i> Mostrar Variaciones entre las Fechas Seleccionadas</button>
                </div>
            </div>
            </form>
            <!--FIN de Selector de Fechas de Inicio y Fin de la consulta-->
            ...
            <div class="row">
                <div class="col-md-12">
                    <table class="table table-bordered table-condensed table-striped" id="correspondencias">
                        ...
                        <tr>
                            <th colspan="3" class="celdacentradahor celdacentradavert nombre_makito bg_th">
                                <strong>Variaciones de Stock agregadas</strong><br><small><span class="salida">Ventas-Reservas (salidas)</span> / <span class="entrada">Compras-Devoluciones (entradas)</span></small>
                            </th>
                            
                            <?php
                            $num_dias = count($array_stocks_dias);
                            $diferencia_stocks_cifra = $array_stocks_dias[0][1] - $array_stocks_dias[$num_dias-1][1];
                            $diferencia_stocks_giving = $array_stocks_dias[0][2] - $array_stocks_dias[$num_dias-1][2];
                            $diferencia_stocks_mob = $array_stocks_dias[0][3] - $array_stocks_dias[$num_dias-1][3];
                            $diferencia_stocks_ps = $array_stocks_dias[0][4] - $array_stocks_dias[$num_dias-1][4];
                            $diferencia_stocks_pf = $array_stocks_dias[0][5] - $array_stocks_dias[$num_dias-1][5];
                            $diferencia_stocks_ggoya = $array_stocks_dias[0][6] - $array_stocks_dias[$num_dias-1][6];
                            ?>
                            
                            <td class="celdacentradahor celdacentradavert celda_homogenea <?php if($incremento_en_stock_cifra == true) {echo "con_entrada";} ?>">
                                <?php
                                    if($diferencia_stocks_cifra >= 0) {
                                        ?>
                                        <span class="salida"><strong><?php echo number_format($diferencia_stocks_cifra, 0, ',', '.'); ?></strong></span>
                                        <?php
                                    } else {
                                        ?>
                                        <span class="entrada"><?php echo number_format($diferencia_stocks_cifra, 0, ',', '.'); ?></span>
                                        <?php
                                    }  
                                ?>
                            </td>
                            <td class="celdacentradahor celdacentradavert celda_homogenea <?php if($incremento_en_stock_giving == true) {echo "con_entrada";} ?>">
                                <?php
                                    if($diferencia_stocks_giving >= 0) {
                                        ?>
                                        <span class="salida"><strong><?php echo number_format($diferencia_stocks_giving, 0, ',', '.'); ?></strong></span>
                                        <?php
                                    } else {
                                        ?>
                                        <span class="entrada"><?php echo number_format($diferencia_stocks_giving, 0, ',', '.'); ?></span>
                                        <?php
                                    }  
                                ?>
                            </td>
                            <td class="celdacentradahor celdacentradavert celda_homogenea <?php if($incremento_en_stock_mob == true) {echo "con_entrada";} ?>">
                                <?php
                                    if($diferencia_stocks_mob >= 0) {
                                        ?>
                                        <span class="salida"><strong><?php echo number_format($diferencia_stocks_mob, 0, ',', '.'); ?></strong></span>
                                        <?php
                                    } else {
                                        ?>
                                        <span class="entrada"><?php echo number_format($diferencia_stocks_mob, 0, ',', '.'); ?></span>
                                        <?php
                                    }  
                                ?>
                            </td>
                            <td class="celdacentradahor celdacentradavert celda_homogenea <?php if($incremento_en_stock_ps == true) {echo "con_entrada";} ?>">
                                <?php
                                    if($diferencia_stocks_ps >= 0) {
                                        ?>
                                        <span class="salida"><strong><?php echo number_format($diferencia_stocks_ps, 0, ',', '.'); ?></strong></span>
                                        <?php
                                    } else {
                                        ?>
                                        <span class="entrada"><?php echo number_format($diferencia_stocks_ps, 0, ',', '.'); ?></span>
                                        <?php
                                    }  
                                ?>
                            </td>
                            <td class="celdacentradahor celdacentradavert celda_homogenea <?php if($incremento_en_stock_pf == true) {echo "con_entrada";} ?>">
                                <?php
                                    if($diferencia_stocks_pf >= 0) {
                                        ?>
                                        <span class="salida"><strong><?php echo number_format($diferencia_stocks_pf, 0, ',', '.'); ?></strong></span>
                                        <?php
                                    } else {
                                        ?>
                                        <span class="entrada"><?php echo number_format($diferencia_stocks_pf, 0, ',', '.'); ?></span>
                                        <?php
                                    }  
                                ?>
                            </td>
                            <td class="celdacentradahor celdacentradavert celda_homogenea <?php if($incremento_en_stock_ggoya == true) {echo "con_entrada";} ?>">
                                <?php
                                    if($diferencia_stocks_ggoya >= 0) {
                                        ?>
                                        <span class="salida"><strong><?php echo number_format($diferencia_stocks_ggoya, 0, ',', '.'); ?></strong></span>
                                        <?php
                                    } else {
                                        ?>
                                        <span class="entrada"><?php echo number_format($diferencia_stocks_ggoya, 0, ',', '.'); ?></span>
                                        <?php
                                    }  
                                ?>
                            </td>
                            <td colspan="3" class="celdacentradahor celdacentradavert nombre_makito bg_th">
                                <small>Variaciones entre las fechas seleccionadas:<br>desde el <strong><?php echo date("d/m/Y", strtotime($fecha_inicio)); ?></strong> hasta el <strong><?php echo date("d/m/Y", strtotime($fecha_fin)); ?></strong></small>
                            </td>
                        </tr>
                    </table>
                </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <h6 class="text-center"><small>notas.-</small> Las Variaciones de Stock aparecerán sobre fondo amarillo si durante el período seleccionado se ha producido alguna entrada (compra o devolución), no reflejando las ventas reales.</h6>
                    <h6 class="text-center">Las Salidas pueden corresponder, en parte o totalmente, a RESERVAS y no a ventas reales. Las Entradas ,a su vez, a RESERVAS CANCELADAS o DEVOLUCIONES y no a compras reales de mercancía.</h6>
                </div>
            </div>
            
            <hr class="sin_margen_sup">
            ...
            <div class="row">
                <div class="col-md-8 col-md-offset-2">
                    <h5 class="text-center enmarcado2">Selector para Nueva Consulta de Variaciones de Stocks del Competidor CIFRA</h5>
                </div>
            </div>
                    ...
                    <form class="form-horizontal" name="selector_ref2" method="post" action="">
                        <div class="form-group">                
                            <select class="form-control" name="filtro_referencia" id="filtro_referencia" required>
                                ...
                                $resultado_consulta_referencias_cifra->MoveFirst(); 
        
                                while (!($resultado_consulta_referencias_cifra->EOF)){
                                    $ref_articulo = $resultado_consulta_referencias_cifra->fields['refs_cifra_referencia_raiz'];
                                    
                                    $consulta_datos_referencia = "SELECT * FROM refs_cifra
                                                                    WHERE refs_cifra_referencia_raiz LIKE '$ref_articulo'
                                                                    LIMIT 1;";
                                    $resultado_consulta_datos_referencia = $bd->Execute($consulta_datos_referencia);
                                    
                                    //Compruebo si la referencia raiz tiene más de una referencia "hija" para en ese caso mostrar el nombre sin la parte correspondiente a las variantes                    
                                    $consulta_num_ref_hijas = "SELECT COUNT(*) FROM refs_cifra
                                                                    WHERE refs_cifra_referencia_raiz LIKE '$ref_articulo';";
                                    $resultado_consulta_num_ref_hijas = $bd->Execute($consulta_num_ref_hijas);
                                    $num_ref_hijas = $resultado_consulta_num_ref_hijas->fields[0];

                                    if($num_ref_hijas > 1) {

                                        $consulta_ref_hijas = "SELECT refs_cifra_nombre FROM refs_cifra
                                                                    WHERE refs_cifra_referencia_raiz LIKE '$ref_articulo';";
                                        $resultado_consulta_ref_hijas = $bd->Execute($consulta_ref_hijas);

                                        $nombre_ref_hija1 = $resultado_consulta_ref_hijas->fields['refs_cifra_nombre'];
                                        $resultado_consulta_ref_hijas->MoveNext();
                                        $nombre_ref_hija2 = $resultado_consulta_ref_hijas->fields['refs_cifra_nombre'];

                                        $array_nombre_ref_hija1 = explode(" ", $nombre_ref_hija1);
                                        $array_nombre_ref_hija2 = explode(" ", $nombre_ref_hija2);

                                        $longitud_array_nombre_ref_hija1 = count($array_nombre_ref_hija1);

                                        $nombre_articulo = "";

                                        $coincide_palabra = true;

                                        $indice = 0;

                                        while($coincide_palabra and ($indice < $longitud_array_nombre_ref_hija1)) {
                                            if($array_nombre_ref_hija1[$indice] == $array_nombre_ref_hija2[$indice]) {
                                                $nombre_articulo = $nombre_articulo.$array_nombre_ref_hija1[$indice]." ";
                                                $indice++;
                                            } else {
                                                $coincide_palabra = false;
                                            }
                                        }

                                    } else {
                                        $nombre_articulo = $resultado_consulta_datos_referencia->fields['refs_cifra_nombre'];
                                    }

                                    echo '<option value="'.$ref_articulo.'">'.$ref_articulo.' - '.$nombre_articulo.'</option>';

                                    $resultado_consulta_referencias_cifra->MoveNext();    
                                }
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                //Script que comprueba que la fecha de inicio seleccionada sea anterior a la de fin seleccionada
                $("#btn_filtrar_fechas").click(function(event) {
                    var fecha_inicio = $("#fecha_inicio").val();
                    var fecha_fin = $("#fecha_fin").val();
                    
                    if(fecha_inicio > fecha_fin) {
                        alert("ATENCIÓN: Debe elegir una Fecha de Inicio ANTERIOR a la Fecha de Fin");
                            event.preventDefault();
                    } else {
                ...
                //FIN del Script que comprueba que la fecha de inicio seleccionada sea anterior a la de fin seleccionada
        ...
        <div class="container" id="cuerpo_home">
            ...
            <div class="row">
                <div class="col-md-12">
                    <h3 class="text-center enmarcado2">Consulta de Variaciones de Stocks del Competidor CIFRA</h3>
                </div>
            </div>
